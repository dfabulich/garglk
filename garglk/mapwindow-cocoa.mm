...
@property (assign) MapWindowController *controller;
...
- (void)mouseUp:(NSEvent *)event {
    ...
    if (self.didDrag)
        return;
    if (!self.controller.mapEventRequest)
        return;
    NSPoint p = [self convertPoint:event.locationInWindow fromView:nil];
...
- (instancetype)initWithGameWindow:(NSWindow *)gameWindow
    ...
    if (self) {
        ...
        if (title.length)
            win.title = [NSString stringWithFormat:@"Map - %@", title];
        else
            win.title = @"Map";
...
- (void)prepareMapWindowFrameBesideGame {
    NSWindow *gameWin = self.gameWindow;
...
- (void)noteMapEventSubtype:(NSUInteger)subtype payload:(NSUInteger)payload {
    if (!self.mapEventRequest)
        return;
    self.mapEventRequest = NO;
    if (self.eventBlock)
        self.eventBlock(subtype, payload);
}
